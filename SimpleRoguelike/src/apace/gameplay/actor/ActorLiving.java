...
public class ActorLiving extends Actor implements IInteractable, Cloneable {
	...
	public void die() {
		isDead = true;
		// TODO: Make this ACTUALLY KILL the actor
	}
	...
	public IProcessable interact(Map map, Actor actor, Position position) {
		...
		if(isDead) {
			...
			return new MultiProcess(new FlashAnimation(this, 10, PaletteSwap.WHITE), new SequenceProcess(new OffsetAnimation(getPosition(), 0, 0, targetX, targetY).setSpeed(0.4f), new OffsetAnimation(getPosition(), targetX, targetY, 0, 0).setSpeed(0.4f)));
